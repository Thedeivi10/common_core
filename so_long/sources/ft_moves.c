...
/*   Updated: 2024/11/27 19:46:41 by davigome         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_move_player(t_map *game, int x, int y)
{
	...
	if (game->grid[new_y][new_x] != '1' && game->grid[new_y][new_x] != 'E')
	{
		...
		if (game->grid[new_y][new_x] == 'C')
			game->collectibles--;
		mlx_image_to_window(game->mlx, game->images.floor, game->player.position.x * 64,
			 game->player.position.y * 64);
		mlx_image_to_window(game->mlx, game->images.floor, new_x * 64, new_y * 64);
		mlx_image_to_window(game->mlx, game->images.player, new_x * 64, new_y * 64);
